...
import { GridOption, IGridColumnConfig, IPagedListRequest, IGridCommand } from './grid-control.model';
...
export class GridControlComponent implements OnInit {
  ...
  reload() {
    ...
    this.apiService.post(this.gridOption.url, pagedListRequest)
      .then(response => {
        this.items = response.json().Entities;
  ...
  resize() {
    $(this.table.nativeElement).height(`calc(100vh - 90px)`);
    setTimeout(() => {
      let top = $(this.table.nativeElement).offset().top;
      $(this.table.nativeElement).height(`calc(100vh - ${top + 90}px)`);
    }, 500);
  }
