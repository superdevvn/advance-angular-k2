...
export class ScheduleControlComponent implements OnInit {
  ...
  private tooltipInfo: ScheduleTooltipInfo = new ScheduleTooltipInfo();
  private option: ScheduleOption;

  private externalEvents: ScheduleExternalEvent[] = [];
  ...
  private resize() {
    setTimeout(() => {
      console.log(this.control.offset().top);
      this.control.height(`${$(window).height() - this.control.offset().top}px`);
      this.control.fullCalendar('option','height',$(window).height() - this.control.offset().top)
    }, 1000);
  }
